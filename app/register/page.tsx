            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                onClick={async () => {
                  ...
                  // We store role and phone in session storage to retrieve them after redirect
                  sessionStorage.setItem('pendingRole', role);
                  setIsLoading(true);
                  try {
                    await authClient.signIn.social({
                      provider: 'google',
                      callbackURL: '/complete-profile', // Redirect to a page that ensures extra info is saved
                    });
                  } catch (err) {
              ...
              <button
                type="button"
                onClick={() => {
                  toast.info('GitHub registration is not yet configured. Please use Email or Google.');
                }}
                className="flex items-center justify-center gap-2 px-4 py-3 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-xl font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-200"
              >
