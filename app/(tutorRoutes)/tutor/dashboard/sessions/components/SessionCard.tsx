  ...
  return (
    <article className="group relative bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-indigo-500/10">
      ...
      <div className={`absolute left-0 top-0 bottom-0 w-1 ${
        isInProgress 
          ? 'bg-linear-to-b from-green-500 to-emerald-500'
          : isCompleted
      ...
      <div className="p-6 lg:p-8">
        <div className="flex flex-col lg:flex-row lg:items-center gap-6">
          ...
          <div className="flex items-center gap-3 lg:ml-auto">
            {(isInProgress || session.status === 'confirmed') && !isCompleted && session.status !== 'cancelled' && (
              <button
                type="button"
                onClick={() => {
                ...
                disabled={isMarkingComplete}
                className={`inline-flex items-center gap-2 px-6 py-3 text-white font-semibold rounded-xl shadow-lg transition-all duration-300 hover:-translate-y-0.5 disabled:hover:translate-y-0 disabled:cursor-not-allowed ${
