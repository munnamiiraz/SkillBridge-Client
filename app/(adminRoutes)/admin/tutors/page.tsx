  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
           <h2 className="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Academic Authority Hub</h2>
        ...
        <div className="flex items-center gap-3">
           <div className="px-4 py-2 bg-indigo-50 dark:bg-indigo-900/20 rounded-2xl border border-indigo-100 dark:border-indigo-800 text-[10px] font-bold uppercase text-indigo-600 tracking-widest flex items-center gap-2 shadow-sm">
              <Clock size={14} />
      ...
      <div className="flex flex-col md:flex-row gap-6">
         ...
         <button className="px-8 bg-white dark:bg-gray-900 rounded-[1.8rem] border border-gray-100 dark:border-gray-800 text-xs font-bold uppercase tracking-widest text-gray-400 hover:text-indigo-600 hover:border-indigo-500/30 transition-all shadow-sm active:scale-95 flex items-center gap-2">
            Advanced Filters
         </button>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
         {loading ? (
            ...
            <div className="col-span-full p-20 text-center font-bold uppercase text-gray-400 tracking-widest">No matching tutors found</div>
         ) : (
            tutors.map((tutor) => (
                  ...
                  <CardContent className="p-10">
                     <div className="flex flex-col sm:flex-row gap-8 items-start">
                        ...
                        <div className="relative shrink-0">
                           ...
                           <div className="relative w-28 h-28 rounded-[2.2rem] bg-white dark:bg-gray-800 border-4 border-white dark:border-gray-900 shadow-xl overflow-hidden text-gray-400">
                              {tutor.image ? <img src={tutor.image} alt={tutor.name} className="w-full h-full object-cover" /> : <span className="w-full h-full flex items-center justify-center text-3xl font-bold bg-gray-50">{tutor.name[0]}</span>}
                           </div>
                           {tutor.tutor_profile.isVerified && (
                        ...
                        <div className="flex-1 min-w-0">
                           <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-4">
                              <h3 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight truncate">{tutor.name}</h3>
                              <span className="shrink-0 px-3 py-1 rounded-xl bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 text-[9px] font-bold uppercase tracking-widest border border-indigo-100 dark:border-indigo-800">
                           ...
                           <div className="flex flex-wrap gap-2 pt-6 border-t border-gray-50 dark:border-gray-800">
                              {tutor.tutor_profile.isVerified ? (
                                 <div className="flex items-center gap-2 text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-tighter">
                                 ...
                                 <>
                                    {isSuper ? (
                                       <button 
                                         ...
                                         className="px-6 py-2.5 bg-linear-to-r from-indigo-600 to-purple-600 text-white rounded-2xl text-[10px] font-bold uppercase tracking-widest shadow-xl shadow-indigo-500/20 hover:scale-[1.05] transition-all active:scale-95 flex items-center gap-2"
                                       >
                                          <CheckCircle size={14} /> Approve Verification
                                       </button>
                                    ) : (
                                       <div className="flex items-center gap-2 text-xs font-bold text-amber-500 uppercase tracking-tighter">
      ...
      {!isSuper && (
         <div className="bg-white/50 dark:bg-gray-900/50 backdrop-blur-md rounded-3xl p-10 border border-gray-100 dark:border-gray-800 flex flex-col md:flex-row gap-8 items-center text-center md:text-left">
            ...
            <div>
               <h4 className="text-xl font-bold text-gray-900 dark:text-white tracking-tight uppercase mb-2">Vetting Authority Alert</h4>
