...
const TutorCard: React.FC<TutorCardProps> = ({ tutor }) => {
  return (
    <Link href={`/tutors/${tutor.id}`} className="block h-full">
      <article className="group relative bg-white dark:bg-gray-800 rounded-[2rem] border border-gray-200 dark:border-gray-700 overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(79,70,229,0.15)] dark:hover:shadow-[0_20px_50px_rgba(79,70,229,0.2)] h-full flex flex-col">
        {/* Card Header with Gradient Background */}
        <div className={`h-24 bg-linear-to-br ${tutor.bgGradient} relative overflow-hidden shrink-0`}>
        ...
        <div className="relative px-6 pb-6 pt-12 flex-1 flex flex-col">
          {/* Avatar Section - Revised for full visibility */}
          <div className="absolute -top-12 left-6 z-10 flex items-end gap-2">
            <div className={`w-20 h-20 rounded-2xl bg-linear-to-br ${tutor.bgGradient} p-1 shadow-2xl`}>
              <div className="w-full h-full rounded-[0.8rem] bg-white dark:bg-gray-800 flex items-center justify-center text-gray-900 dark:text-white font-bold text-2xl border-2 border-white dark:border-gray-800 overflow-hidden">
                {tutor.avatar.length > 2 ? (
                  <img src={tutor.avatar} alt={tutor.name} className="w-full h-full object-cover" />
                ) : (
                  <span className={`bg-linear-to-br ${tutor.bgGradient} bg-clip-text text-transparent font-black`}>{tutor.avatar}</span>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="absolute top-4 right-6 text-right">
              <div className="flex items-center gap-1.5 bg-yellow-50/90 dark:bg-yellow-900/40 backdrop-blur-md px-3 py-1 rounded-full border border-yellow-100 dark:border-yellow-800 shadow-sm">
                <svg className="w-3.5 h-3.5 text-yellow-500 fill-current" viewBox="0 0 20 20">
                  <path d="M10 15l-5.878 3.09 1.123-6.545L.489 6.91l6.572-.955L10 0l2.939 5.955 6.572.955-4.756 4.635 1.123 6.545z"/>
                </svg>
                <span className="text-xs font-bold text-gray-900 dark:text-white">{Number(tutor.rating).toFixed(1)}</span>
                <span className="text-[10px] text-gray-400 dark:text-gray-500">({tutor.reviewCount})</span>
              </div>
          </div>
          ...
          <div className="flex-1 flex flex-col pt-2">
            <div className="mb-4">
              <div className="flex items-center justify-between gap-4 mb-2">
                <h3 className="text-xl font-bold text-gray-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors duration-300 truncate tracking-tight">
                  {tutor.name}
                </h3>
                {tutor.verified && (
                  <div className="shrink-0 w-6 h-6 bg-blue-500 rounded-full flex items-center justify-center text-white shadow-lg shadow-blue-500/20 border-2 border-white dark:border-gray-800">
              ...
              <div className="flex items-center gap-1.5">
                <span className="px-2.5 py-0.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 text-[10px] font-bold uppercase tracking-wide rounded-lg border border-indigo-100/50 dark:border-indigo-800/50">
                  {tutor.subject}
                </span>
                <span className="w-1 h-1 rounded-full bg-gray-300 dark:bg-gray-600" />
                <span className="text-[10px] text-gray-500 dark:text-gray-500 font-bold uppercase tracking-tight">
            ...
            <div className="mt-auto pt-5 flex items-center justify-between border-t border-gray-100 dark:border-gray-700/50">
              <div>
                <p className="text-[9px] uppercase tracking-widest font-black text-gray-400 dark:text-gray-500 mb-1">Session Price</p>
                <div className="text-2xl font-black text-gray-900 dark:text-white leading-none">
                  ${tutor.pricePerSession}<span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-tighter ml-1">/hr</span>
                </div>
              </div>
              
              <div className="px-5 py-3 bg-linear-to-br from-indigo-600 to-purple-600 text-white font-bold text-sm rounded-2xl shadow-xl shadow-indigo-500/20 group-hover:scale-[1.05] active:scale-[0.98] transition-all">
                View Profile
              </div>
            </div>
          </div>
