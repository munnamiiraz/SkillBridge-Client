    ...
    <section className="relative w-full py-24 bg-linear-to-b from-white to-gray-50 dark:from-gray-950 dark:to-gray-900 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16 space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-full text-[10px] font-black border border-indigo-100 dark:border-indigo-800 tracking-widest uppercase">
          ...
          <h2 className="text-3xl lg:text-4xl font-black text-gray-900 dark:text-white leading-tight tracking-tight">
            ...
            <span className="bg-linear-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent underline decoration-indigo-500/20 underline-offset-4">They Succeed.</span>
          </h2>
        </div>

        <div className="relative max-w-4xl mx-auto">
          {/* Main Carousel Card */}
          <div className="relative bg-white dark:bg-gray-800 rounded-[2.5rem] border border-gray-100 dark:border-gray-700 shadow-2xl p-8 sm:p-12 min-h-[350px] flex flex-col justify-center items-center text-center group">
            
            {/* Navigation Buttons */}
            <button onClick={prev} className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 flex items-center justify-center text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-white transition-all shadow-sm md:-left-5">
              <ArrowLeft className="w-5 h-5" />
            </button>
            <button onClick={next} className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-gray-50 dark:bg-gray-900 border border-gray-100 dark:border-gray-700 flex items-center justify-center text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-white transition-all shadow-sm md:-right-5">
              <ArrowRight className="w-5 h-5" />
            </button>

            <div key={current} className="space-y-6 animate-in fade-in zoom-in duration-700">
              {/* Badge */}
              <div className="inline-block px-3 py-1 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 text-[10px] font-black rounded-full uppercase tracking-widest border border-emerald-100 dark:border-emerald-800">
                Verified Achievement
              </div>

              {/* Quote Area */}
              <div className="relative px-4">
                <Quote className="absolute -top-4 -left-2 w-10 h-10 text-gray-100 dark:text-gray-700 -z-10" />
                <h3 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white leading-relaxed italic">
